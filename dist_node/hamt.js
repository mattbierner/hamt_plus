...
var Leaf = (function(edit, hash0, key, value) {
    ...
    updateCollisionList = (function(eq, list, f, k) {
        var first, rest, v;
        return ((!list.length) ? [] : ((first = list[0]), (rest = list.slice(1)), (eq(first.key, k) ? ((v = f(first
            .value)), ((nothing === v) ? rest : [v].concat(rest))) : [first].concat(updateCollisionList(
            rest, f, k)))));
    }),
...
(IndexedNode.prototype.modify = (function(eq, edit, shift, f, h, k) {
    var self = this,
        ...
        child = alter(false, eq, edit, (exists ? children[indx] : null), (shift + 5), f, h, k),
    ...
    return ((!bitmap0) ? null : (removed ? (((children.length <= 2) && isLeaf(children[(indx ^ 1)])) ? children[
        (indx ^ 1)] : new(IndexedNode)(edit, bitmap0, arraySpliceOut(indx, self.children))) : (added ?
        ((self.children.length >= MAX_INDEX_NODE) ? expand(edit, frag, child, self.mask, children) :
            new(IndexedNode)(edit, bitmap0, arraySpliceIn(indx, child, children))) : new(IndexedNode)(
            edit, bitmap0, arrayUpdate(indx, child, children)))));
}));
(IndexedNode.prototype.mutate = (function(eq, edit, shift, f, h, k) {
    var self = this,
        children = self["children"],
        bitmap;
    if ((edit != self.edit)) return self.modify(eq, edit, shift, f, h, k);
    var frag = ((h >>> shift) & mask),
        ...
        exists = (self.mask & bit),
        child = alter(true, eq, edit, (exists ? children[indx] : null), (shift + 5), f, h, k),
        removed = (exists && (!child)),
        added = ((!exists) && (!(!child))),
        bitmap0 = (removed ? (self.mask & (~bit)) : (added ? (self.mask | bit) : self.mask));
    if ((!bitmap0)) {
...
(ArrayNode.prototype.modify = (function(eq, edit, shift, f, h, k) {
    var self = this,
        frag = ((h >>> shift) & mask),
        child = self.children[frag],
        newChild = alter(false, eq, edit, child, (shift + 5), f, h, k);
    return (((!child) && (!(!newChild))) ? new(ArrayNode)(edit, (self.count + 1), arrayUpdate(frag, newChild,
        self.children)) : (((!(!child)) && (!newChild)) ? (((self.count - 1) <= MIN_ARRAY_NODE) ? pack(edit,
            frag, self.children) : new(ArrayNode)(edit, (self.count - 1), arrayUpdate(frag, null, self.children))) :
        new(ArrayNode)(edit, self.count, arrayUpdate(frag, newChild, self.children))));
}));
...
(alter = (function(mutate0, eq, edit, n, shift, f, h, k) {
    var v;
    return ((!n) ? ((v = f()), ((nothing === v) ? null : new(Leaf)(edit, h, k, v))) : (mutate0 ? n.mutate(eq,
        edit, shift, f, h, k) : n.modify(eq, edit, shift, f, h, k)));
}));
...
(modify = (function(k, f, m) {
    ...
    return Tree.setRoot(((mutate0 = m.mutable), (eq = m.config.keyEq), (edit = m.edit), (n = m.root), (h = m.config
        .hash(k)), ((!n) ? ((v = f()), ((nothing === v) ? null : new(Leaf)(edit, h, k, v))) : (mutate0 ?
        n.mutate(eq, edit, 0, f, h, k) : n.modify(eq, edit, 0, f, h, k)))), m);
}));
(set = (function(k, v, m) {
    var f = (function() {
        return v;
        ...
        mutate0, eq, edit, n, h;
    return Tree.setRoot(((mutate0 = m.mutable), (eq = m.config.keyEq), (edit = m.edit), (n = m.root), (h = m.config
        .hash(k)), ((!n) ? ((nothing === v) ? null : new(Leaf)(edit, h, k, v)) : (mutate0 ? n.mutate(eq,
        edit, 0, f, h, k) : n.modify(eq, edit, 0, f, h, k)))), m);
}));
var del = (function() {
    return nothing;
});
(remove = (function(k, m) {
    ...
    return Tree.setRoot(((mutate0 = m.mutable), (eq = m.config.keyEq), (edit = m.edit), (n = m.root), (h = m.config
        .hash(k)), ((!n) ? ((nothing === nothing) ? null : new(Leaf)(edit, h, k, nothing)) : (mutate0 ?
        n.mutate(eq, edit, 0, del, h, k) : n.modify(eq, edit, 0, del, h, k)))), m);
}));

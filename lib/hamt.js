...
*/
const updateCollisionList = (edit, keyEq, h, list, f, k) => {
    ...
    for (let i = 0; i < len; ++i) {
        ...
        if (keyEq(k, child.key)) {
            ...
            return newValue === nothing
                ...
                :arrayUpdate(false, i, Leaf(edit, h, k, newValue), list);
        }
    }
    ...
    return newValue === nothing
        ?list
        :arrayUpdate(false, len, Leaf(edit, h, k, newValue), list);
};
...
const Leaf__modify = function(edit, keyEq, shift, f, h, k) {
   if (keyEq(k, this.key)) {
    ...
    return v === nothing
        ?this
        :mergeLeaves(edit, shift, this.hash, this, h, Leaf(edit, h, k, v));
};

const Collision__modify = function(edit, keyEq, shift, f, h, k) {
    if (h === this.hash) {
        const list = updateCollisionList(edit, keyEq, this.hash, this.children, f, k);
...
const IndexedNode__modify = function(edit, keyEq, shift, f, h, k) {
    ...
    if (!exists && !isEmptyNode(child)) { // add
        return children.length >= MAX_INDEX_NODE
            ?expand(edit, frag, child, mask, children)
            :IndexedNode(
                edit,
                mask | bit,

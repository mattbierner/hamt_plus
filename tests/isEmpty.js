...
describe('isEmpty', () => {
    it('should return true for empty', () => {
        assert.isTrue(hamt.isEmpty(hamt.make()));
        assert.isTrue(hamt.make().isEmpty());
    });
    
    it('should false for single element map', () => {
        assert.isFalse(hamt.isEmpty(hamt.make().set('a', 4)));
        assert.isFalse(hamt.make().set('a', 4).isEmpty());
    });
